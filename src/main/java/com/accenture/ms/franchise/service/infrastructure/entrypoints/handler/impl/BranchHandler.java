...
import com.accenture.ms.franchise.service.infrastructure.entrypoints.mapper.UpdateBranchRequestMapper;
...
public class BranchHandler implements IBranchHandler {
    ...
    @Override
    public Mono<BranchResponseDTO> updateBranchName(BranchUpdateRequestDTO branchUpdateRequestDTO) {
        return Mono.just(branchUpdateRequestDTO)
                .map(updateBranchRequestMapper::toModel)
                .flatMap(franchiseServicePort::updateBranch)
                .map(branchResponseMapper::toResponse)
                .onErrorMap(ex -> {
